...
<body>
    <main class="lg:px-20 sm:px-2 md:px-10">
        ...
        <main class="mb-10 p-10">
            <form action="/dashboard/course/update?id=<?= $model['data']['id'] ?>" method="POST" >
                <div class="relative z-0 w-full mb-6 group">
                    <input value="<?= $model['data']['title'] ?>" type="text" name="title" id="title" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
                ...
                <div class="relative z-0 w-full mb-6 group">
                    <input value="<?= $model['data']['description'] ?>" type="text" name="description" id="description" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
                ...
                <select id="catagory" name="category_id" class="mb-5 border bg-white border-black text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                    ...
                    <option value="<?= $value['id']?>" <?php echo ($value['id'] == $model['data']['category_id']) ? 'selected' : ''; ?> ><?= $value['name'] ?></option>
                    <?php } ?>
                </select>
